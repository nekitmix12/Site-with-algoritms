...
function createButtonBlock(){
    ...
    buttonBlock.title = 'Stop';
    buttonBlock.textContent = 'Stop';
    buttonBlock.style.zIndex = '2';
    buttonBlock.style.height = '40px';
    buttonBlock.style.width = '100px';
    buttonBlock.style.position = 'absolute';
    buttonBlock.style.top = '25%';
    buttonBlock.style.right = '5%';
    buttonBlock.style.backgroundColor = '#1D1F20';
    buttonBlock.style.color = '#FFFFFF';
    buttonBlock.style.border = 'none';
    buttonBlock.style.borderRadius = '20px';
    buttonBlock.style.boxShadow = '0px 8px 15px rgba(0, 0, 0, 0.1)';
    buttonBlock.style.transition = 'all 0.3s ease 0s';
    buttonBlock.style.cursor = 'pointer';
    buttonBlock.style.textAlign = 'center';
    buttonBlock.onmouseover = function () {
        this.style.backgroundColor = 'blue';
    };
    buttonBlock.onmouseout = function () {
        this.style.backgroundColor = '#1D1F20';
    };
...
function createNumInput(id = 'numField',ph = 'Count clusters',top = "45%",left = "5%") {
...
function createVisualizationAnt(functionName) {
    ...
    createColorUser();
    createButtonBlock();
    createNumInput("alpha","write alpha","45%","5%");
    createNumInput("betta","write Betta","48%","5%");
    createNumInput("pheromone","write Pheromone","51%","5%")
    createNumInput("iteration","write Iteration","54%","5%")
    creatArea(functionName);
}
...
function deleteVisualizationAnt() {
    ...
    deleteColorUser();
    deleteButtonBlock();
    deleteElementById("alpha");
    deleteElementById("betta");
    deleteElementById("iteration");
    deleteElementById("pheromone");
}
    ...
    switch (functionName) {
        ...
        case('ant'):
            ...
            if(points.length ===0)
                ...
                let betta = document.getElementById("betta");
                let iteration = document.getElementById("iteration");
                let pheromone = document.getElementById("pheromone");
                if(alpha.value==="")alert("введите альфа-значение");
                else if(betta.value==="")alert("введите бетта-значение");
                else if(iteration.value==="")alert("введите количество итераций");
                else if(pheromone.value==="")alert("введите скорость испарения");
                else antAlgorithm(points,iteration.value,0.6,alpha.value,betta.value,pheromone.value);
            }
            break;
        case ('cluster'):
